...
import { filter, map, reduce } from "generic-handler/built_in_generics/generic_collection"
import { construct_better_set, identify_by, is_better_set, set_merge, type BetterSet } from "generic-handler/built_in_generics/generic_better_set"
...
export function construct_layered_procedure_metadata(name: string, arity: number, base_proc: (...args: any) => any): LayeredProcedureMetadata {
    var handlers = new Map<string, (b: any, ...v: any) => any>();
    ...
    function get_handler(layer: Layer<any>): ((b: any, ...v: any) => any)  {
        ...
        const handler = handlers.get(layerName);
        if (handler) {
            return handler;
        } else {
...
function layered_procedure_dispatch<T>(metaData: LayeredProcedureMetadata, ...args: any[]) {
        ...
        return construct_layered_object<T>(
            ...
            pipe(annotation_layers, 
                (s: BetterSet<Layer<any>>) => filter(s, (l: Layer<any>) => l.get_name() !== "base"),
                (s: BetterSet<Layer<any>>) => map(s, (layer: Layer<any>): [Layer<any>, any] => {
