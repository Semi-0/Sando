...
import { construct_layered_object, make_layered_alist } from "./LayeredObject";
...
export const plist_to_alist = (plist: any[]) => {
    ...
    const loop = (index: number, acc: BetterSet<[Layer<any>, any]>): BetterSet<[Layer<any>, any]> => {
        ...
        return loop(index + 2, set_add_item(acc, [plist[index], plist[index + 1]]))
    }
    return loop(0, make_layered_alist([]))
}
